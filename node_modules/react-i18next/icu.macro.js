...
function ICUMacro({ references, state, babel }) {
  ...
  Object.entries({
    ...
    node.forEach((item) => {
      ...
      while (f) {
        if (babel.types.isJSXElement(f.node)) {
          ...
          const isMemberExpression =
            babel.types.isJSXMemberExpression(elementName);
      ...
      if (!loc) {
        throw new Error(
          `"${name}\`\`" can only be used inside <Trans> in "unknown file"`,
        );
      }
      throw new Error(
...
function pluralAsJSX(parentPath, { attributes }, babel) {
  ...
  const extracted = attributes.reduce(
    (mem, attr) => {
      if (attr.node.name.name === "i18nKey") {
        ...
        if (pluralForm.indexOf("$") === 0)
          pluralForm = pluralForm.replace("$", "=");
  ...
  parentPath.replaceWith(
    buildTransElement(extracted, extracted.attributesToCopy, t, true),
  );
}
  ...
  if (!attributeExistsAlready("defaultTranslation", finalAttributes)) {
    if (extracted.defaults.includes('"')) {
      ...
      finalAttributes.push(
        t.jSXAttribute(
          t.jSXIdentifier("defaultTranslation"),
          t.stringLiteral(extracted.defaults),
        ),
      );
    }
  }
  ...
  if (!attributeExistsAlready("content", finalAttributes))
    finalAttributes.push(
      t.jSXAttribute(
        ...
        t.jSXExpressionContainer(content),
      ),
    );
  ...
  if (!attributeExistsAlready("values", finalAttributes))
    finalAttributes.push(
      t.jSXAttribute(
        ...
        t.jSXExpressionContainer(values),
      ),
    );
...
function jsxElementToDeclaration(jsxElement, t) {
  ...
  if (!jsxElement || !jsxElement.openingElement) {
    console.error(
      "Invalid JSXElement passed to jsxElementToDeclaration:",
      jsxElement,
    );
    return t.objectExpression([
      t.objectProperty(t.identifier("type"), t.stringLiteral("div")),
    ]);
  }
    ...
    if (childDeclarations.length > 0) {
      propsProperties.push(
        t.objectProperty(
          ...
          t.arrayExpression(childDeclarations),
        ),
      );
    }
  }
...
function mergeChildren(children, babel, componentStartIndex = 0) {
  ...
  return children.reduce((mem, child) => {
    ...
    if (ele.expression && ele.expression.name)
      result += `{${ele.expression.name}}`;
    // add `{ val, number }`
    result += mergeCommaExpressions(ele);
    const [nextText, newIndex] = mergeTaggedTemplateExpressions(
      ele,
      componentFoundIndex,
      t,
      babel,
...
function getValues(children, babel) {
  ...
  return children.reduce((mem, child) => {
    ...
    // add `{ var }` to values
    if (ele.expression && ele.expression.name)
      mem.push(toObjectProperty(ele.expression.name));
    // add `{ var, number }` to values
    if (ele.expression && ele.expression.expressions)
      result.push(
        toObjectProperty(
          ele.expression.expressions[0].name ||
            ele.expression.expressions[0].value,
        ),
      );
    ...
    result = result.concat(
      extractTaggedTemplateValues(ele, babel, toObjectProperty),
    );
...
function addNeededImports(state, babel, references) {
  ...
  const existingImport = state.file.path.node.body.find(
    (importNode) =>
      ...
      importNode.source.value === "react-i18next",
  );
...
const extractNestedTemplatesAndComponents = (
  ...
  if (node.type === "JSXElement") {
    ...
    const tagName =
      babel.types.isIdentifier(node.tag) && node.tag.name
        ? node.tag.name
        : "unknown";
    const [variableNames, childText, newIndex] =
...
function getTextAndInterpolatedVariables(type, primaryNode, index, babel) {
  ...
  const { text, interpolatedVariableNames } =
    extractVariableNamesFromQuasiNodes(primaryNode, babel);
